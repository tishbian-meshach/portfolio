...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof skillsConfig> = {}) {
    const config = { ...skillsConfig, ...params };
...
interface CreateSkillsBallpitReturn {
  three: X;
  capsules: Z;
  dispose: () => void;
}

function createSkillsBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateSkillsBallpitReturn {
  ...
  threeInstance.camera.position.set(0, 0, 12);
  threeInstance.camera.lookAt(0, 0, 0);
  threeInstance.cameraMaxAspect = 1.2;
  ...
  const pointerData = createPointerData({
    ...
    onMove() {
      ...
      capsules.physics.center.copy(intersectionPoint);
      capsules.config.controlSphere0 = true;
    },
    onLeave() {
      capsules.config.controlSphere0 = false;
    },
  });
  function initialize(cfg: any) {
    if (capsules) {
      threeInstance.clear();
      threeInstance.scene.remove(capsules);
    }
    capsules = new Z(threeInstance.renderer, cfg);
    threeInstance.scene.add(capsules);
  }
  threeInstance.onBeforeRender = (deltaInfo) => {
    capsules.update(deltaInfo);
  };
  threeInstance.onAfterResize = (size) => {
    capsules.config.maxX = size.wWidth / 2;
    capsules.config.maxY = size.wHeight / 2;
  };
  return {
    three: threeInstance,
    get capsules() {
...
const SkillsBallpit: React.FC<SkillsBallpitProps> = ({
  ...
  const instanceRef = useRef<CreateSkillsBallpitReturn | null>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    instanceRef.current = createSkillsBallpit(canvas, {
      followCursor,
      ...props,
    });
